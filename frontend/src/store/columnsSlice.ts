...
export {
  DEFAULT_VIEW_SETTINGS as COLUMNS_DEFAULT_VIEW_SETTINGS,
  defaultColumns as COLUMNS_DEFAULT_COLUMNS,
...
const columnsSlice = createSlice({
  ...
  reducers: {
    updateColumn(
      state,
      action: PayloadAction<{ view: 'table' | 'list' | 'grid'; key: string; visible: boolean }>
    ) {
    ...
    resetColumns(state, action: PayloadAction<{ view: 'table' | 'list' | 'grid' }>) {
      ...
      state.views[view].settings.rankVariationLocation =
        DEFAULT_VIEW_SETTINGS[view].rankVariationLocation;
      state.views[view].settings.playsVariationDisplay =
        DEFAULT_VIEW_SETTINGS[view].playsVariationDisplay;
      state.views[view].settings.playsVariationLocation =
        DEFAULT_VIEW_SETTINGS[view].playsVariationLocation;
      ...
      state.views[view].settings.listPeakWeeksCombined =
        DEFAULT_VIEW_SETTINGS[view].listPeakWeeksCombined;
      state.views[view].settings.showDroppedItems = DEFAULT_VIEW_SETTINGS[view].showDroppedItems;
      state.views[view].settings.showFormulaInsteadOfPlays =
        DEFAULT_VIEW_SETTINGS[view].showFormulaInsteadOfPlays;
      state.views[view].settings.showCarousel = DEFAULT_VIEW_SETTINGS[view].showCarousel;
      state.views[view].columns = applyRankVariationMapping(
        state.views[view].columns,
        state.views[view].settings.rankVariationLocation!,
        view
      );
      state.views[view].columns = applyPlaysVariationDisplay(
        state.views[view].columns,
        state.views[view].settings.playsVariationDisplay || 'percent',
        state.views[view].settings.playsVariationLocation ||
          DEFAULT_VIEW_SETTINGS[view].playsVariationLocation,
        view
      );
      state.views[view].columns = applyArtistDisplayMode(
        state.views[view].columns,
        state.views[view].settings.artistDisplayMode || 'under',
        view
      );
      // persistence is handled by redux-persist; avoid writing to localStorage here
    },
    setPeakCountStyle(
      state,
      action: PayloadAction<{ view: 'table' | 'list' | 'grid'; mode: 'withCount' | 'noCount' }>
    ) {
      ensureViews(state as any);
    ...
    setContainerSize(
      state,
      action: PayloadAction<{ view: 'table' | 'list' | 'grid'; size: 'md' | 'lg' | 'xl' | '100%' }>
    ) {
      ensureViews(state as any);
    ...
    setRankVariationLocation(
      ...
      const artistMode =
        state.views[view].settings.artistDisplayMode ||
        DEFAULT_VIEW_SETTINGS[view].artistDisplayMode ||
        'under';
      state.views[view].columns = applyArtistDisplayMode(
        state.views[view].columns,
        artistMode,
    ...
    setPlaysVariationDisplay(
      state,
      action: PayloadAction<{ view: 'table' | 'list'; display: 'hidden' | 'absolute' | 'percent' }>
    ) {
      ensureViews(state as any);
    ...
    setPlaysVariationLocation(
      state,
      action: PayloadAction<{ view: 'table' | 'list'; location: 'hidden' | 'under' | 'column' }>
    ) {
      ensureViews(state as any);
    ...
    setArtistDisplayMode(
      state,
      action: PayloadAction<{ view: 'table'; mode: 'under' | 'column' }>
    ) {
      ensureViews(state as any);
      const { view, mode } = action.payload;
    ...
    setFontScale(
      state,
      action: PayloadAction<{ view: 'table' | 'list' | 'grid'; scale: -2 | -1 | 0 | 1 | 2 }>
    ) {
      ensureViews(state as any);
    ...
    setShowDroppedItems(
      state,
      action: PayloadAction<{ view: 'table' | 'list' | 'grid'; show: boolean }>
    ) {
      ensureViews(state as any);
