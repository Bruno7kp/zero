...
export const fetchChartData = createAsyncThunk(
  ...
  async (
    { chartId, chartType, week }: { chartId: string; chartType: string; week: string },
    { dispatch, getState }
  ) => {
    ...
    setTimeout(async () => {
      try {
        ...
        const hasRich = Object.values(st.charts.statsMap || {}).some(
          (s: any) =>
            s &&
            s.chartId == String(chartId) &&
            s.chartType === chartType &&
            s.totals &&
            s.totals.withinCutoff > 1
        );
        if (hasRich) return;
        ...
        dispatch(
          fetchStatsMapIncremental({ chartId: String(chartId), chartType, data: rows, week })
        );
      } catch {
        /* silencioso */
      }
  ...
  async (
    {
      ...
      rows,
      cutoff = 100,
    }: { chartId: string; chartType: string; week: string; rows: ChartData[]; cutoff?: number },
    { dispatch }
  ) => {
    const updates: Array<{
      entityId: string;
      deltaRank: string | number;
      deltaPlays: string | number;
    }> = [];
    for (const r of rows) {
...
export const fetchStatsMapIncremental = createAsyncThunk(
  ...
  async (
    { chartId, chartType, data, week, batchSize = 15 }: IncrementalArgs,
    { dispatch, getState }
  ) => {
    ...
    if (!state0.charts.statsCache || typeof state0.charts.statsCache !== 'object')
      state0.charts.statsCache = {};
    ...
    for (const k of Object.keys(state0.charts.statsCache || {})) {
      try {
        ...
        if (age > STATS_CACHE_TTL_MS + STATS_CACHE_STALE_GRACE_MS)
          dispatch({ type: 'charts/removeStatsCacheEntry', payload: k });
      } catch {
        /* ignore */
      }
    }
    ...
    if (cached) {
      ...
      if (age < STATS_CACHE_TTL_MS) {
        dispatch({ type: 'charts/replaceStatsSnapshot', payload: cached.data });
        return;
      ...
      try {
        ...
        for (const id of existingIds) {
          const s = statsMap[id];
          if (s && s.chartId == chartId && s.chartType === chartType && s._status !== 'stub')
            haveExisting++;
        }
    ...
    dispatch({
      type: silentRevalidate ? 'charts/beginStatsRevalidation' : 'charts/startStatsIncremental',
      payload: requestId,
    });
    ...
    if (entityIds.length) {
      try {
        const compoundKeys = entityIds.map(
          id => [chartId, chartType, id] as [string, string, string]
        );
    ...
    if (data.length <= 60) {
      ...
      if (mode === 'global') {
        ...
        for (const [id, rows] of perEntity) {
          ...
          if (minimal) {
            ...
            if (!existing || existing._status === 'stub') shouldWrite = true;
              ...
              if (
                newWeeks > oldWeeks ||
                (newPeak != null && (oldPeak == null || newPeak < oldPeak))
              )
                shouldWrite = true;
            }
            if (shouldWrite) {
              const partialUpdate = {
                [id]: { chartId, chartType, entityId: id, ...minimal, _status: 'minimal' },
              };
              const enriched = mergeWithFullStats(partialUpdate);
              if (Object.keys(enriched || {}).length) {
                dispatch({
                  type: 'charts/partialStatsLoaded',
                  payload: { requestId, partial: enriched },
                });
              }
            }
          }
        ...
        const isSilent =
          stSilent.charts.revalidatingStats && stSilent.charts.statsRequestId === requestId;
        if (!isSilent) {
          ...
          if (Object.keys(stubPartial).length && stSilent.charts.statsRequestId === requestId) {
            ...
            if (Object.keys(enrichedStub || {}).length) {
              dispatch({
                type: 'charts/partialStatsLoaded',
                payload: { requestId, partial: enrichedStub },
              });
            }
          }
        }
        await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
        const currentWeekMap = new Map<string, ChartData>(
          data.filter(r => r.week === week).map(r => [r.entityId, r])
        );
        ...
        await new Promise<void>(resolve => {
          const launchNext = () => {
            if (idx >= entities.length && active === 0) {
              resolve();
              return;
            }
                ...
                try {
                  ...
                  if (cw) {
                    ...
                    if (existingIdx === -1) {
                      ...
                      if (
                        typeof cw.rank === 'number' &&
                        (existing.rank == null ||
                          (typeof existing.rank === 'number' && cw.rank < existing.rank))
                      ) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (minimal) {
            ...
            if (!existing) shouldWrite = true;
              ...
              else if (newPeak != null && (oldPeak == null || newPeak < oldPeak))
                shouldWrite = true;
            }
        ...
        if (st.charts.statsRequestId === requestId) {
          ...
          if (Object.keys(enrichedSnapshot || {}).length) {
            dispatch({
              type: 'charts/partialStatsLoaded',
              payload: { requestId, partial: enrichedSnapshot },
            });
          }
          dispatch({ type: 'charts/finishStatsIncremental', payload: requestId });
          if (Object.keys(enrichedSnapshot || {}).length) {
            dispatch({
              type: 'charts/cacheStatsSnapshot',
              payload: { cacheKey, snapshot: enrichedSnapshot, now: Date.now() },
            });
          }
        }
        return;
    ...
    for (const [entityId, rows] of Object.entries(rowsBy))
      upsertRunCache(chartId, chartType, entityId, rows);
    ...
    for (let i = 0; i < entities.length; ) {
      ...
      const st: any = getState();
      if (st.charts.statsRequestId !== requestId) return;
    ...
    dispatch({
      type: 'charts/cacheStatsSnapshot',
      payload: { cacheKey, snapshot: finalState.charts.statsMap, now: Date.now() },
    });
  }
);
...
export const deleteChart = createAsyncThunk(
  ...
  async (chartId: number, { dispatch, getState, rejectWithValue }) => {
    try {
      const st: any = getState();
      const token = getToken(st);
      const res = await fetch(apiUrl(`/charts/${chartId}`), {
        method: 'DELETE',
      ...
      try {
        await db.chart_weeks.where('chartId').equals(String(chartId)).delete();
      } catch {
        /* ignore old schema */
      }
