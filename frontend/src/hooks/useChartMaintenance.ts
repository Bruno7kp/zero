...
export function useChartMaintenance(t: any) {
  ...
  const updateState = (chartId: string, patch: ProcessingState | null) => {
    setProcessing(prev => {
      const next = { ...prev };
      if (patch === null) delete next[chartId];
      else next[chartId] = { ...(next[chartId] || {}), ...patch };
      return next;
    });
  };

  const clearChartDataWithConfirm = useCallback(
    (chartId: number, chartName: string) => {
      modals.openConfirmModal({
        title: t('settings.clearChartData') + '? (' + chartName + ')',
        size: 'lg',
        labels: {
          confirm: t('settings.clearChartData'),
        ...
        onConfirm: async () => {
          ...
          notifications.show({
            id: notifId,
            message: t('settings.clearChartProgress', { chart: chartName }),
            loading: true,
            autoClose: false,
          });
          try {
            ...
            notifications.update({
              id: notifId,
              message: t('notifications.cache.chartCleared', { chart: chartName }),
              loading: false,
              color: 'green',
              autoClose: 2500,
            });
  ...
  const rebuildStatsWithConfirm = useCallback(
    (chartId: number, chartName: string) => {
      modals.openConfirmModal({
        ...
        onConfirm: async () => {
          ...
          notifications.show({
            id: notifId,
            message: t('settings.rebuildStatsProgress', { chart: chartName, current: 0, total: 0 }),
            loading: true,
            autoClose: false,
          });
          try {
            ...
            for (const type of types) {
              ...
              const cutoff =
                type === 'artist'
                  ? chartObj?.artist_cutoff || 100
                  : type === 'album'
                  ? chartObj?.album_cutoff || 100
                  : chartObj?.music_cutoff || 100;
              for (const entityId of Object.keys(byEntity)) {
                ...
                if (processed % 5 === 0 || processed === totalEntities) {
                  notifications.update({
                    id: notifId,
                    message: t('settings.rebuildStatsProgress', {
                      chart: chartName,
                      current: processed,
                      total: totalEntities,
                    }),
            ...
            notifications.update({
              id: notifId,
              message: t('settings.rebuildStatsDone', { chart: chartName }),
              loading: false,
              color: 'green',
              autoClose: 2500,
            });
            ...
            notifications.update({
              id: notifId,
              message: t('settings.rebuildStatsFailed') + ' ' + (e?.message || ''),
              loading: false,
              color: 'red',
              autoClose: 5000,
            });
  ...
  const isProcessing = useCallback(
    (chartId: number | string) =>
      !!processing[String(chartId)] &&
      (processing[String(chartId)].clearing || processing[String(chartId)].rebuilding),
    [processing]
  );
