...
function resolveDelta(
  ...
  currentValue?: number,
  darker?: boolean,
  computePercentFn = computeDefaultPercent
) {
  let color = 'gray';
  ...
  if (typeof delta === 'number') {
    ...
    color = darker ? 'cobalt' : 'lazuli';
    label = 'NEW';
  } else if (delta === 'RE') {
    color = darker ? 'honey' : 'bee';
    label = 'RE';
...
export const DeltaBadge: React.FC<DeltaBadgeProps> = ({
  ...
  const { color, label } = resolveDelta(
    effectiveDelta,
    showPercent,
    currentValue,
    darker,
    computePercent || computeDefaultPercent
  );
  ...
  if (
    variant === 'filled' &&
    color === 'gray' &&
    (contextView === 'table' || contextView === 'list' || !contextView)
  )
    variant = 'light';
  ...
  const paddingStyle = noSidePadding
    ? { paddingInline: 0 }
    : cfg.condensed
    ? { paddingInline: 4 }
    : {};
  ...
  const icon = ((): React.ReactNode => {
    ...
    const baseIconSize =
      size === 'xs' ? 12 : size === 'sm' ? 14 : size === 'md' ? 16 : size === 'lg' ? 18 : 20;
    ...
    if (effectiveDelta === '=' && cfg.iconPosition === 'split')
      return <IconSquareFilled size={8} />;
    return null;
  })();
  ...
  const splitVariant = resolveDelta(
    effectiveDelta,
    showPercent,
    currentValue,
    !isFilledVariant,
  ...
  if (cfg.iconPosition === 'split' && icon) {
        ...
        <Flex direction="row" gap={4} align="center" style={{ height: 40 }}>
          ...
          <Badge
            ...
            style={{
              ...
              alignItems: 'stretch',
              ...(isTransparent &&
              (cfg.splitIconVariant === 'transparent' || cfg.splitIconVariant === 'filled')
                ? frostedStyles
                : {}),
            }}
          >
    ...
    return (
      <Flex direction="row" gap={2} align="center">
        ...
        <Badge
          variant={cfg.splitIconVariant || 'filled'}
          color={color}
          radius={radius as any}
          size={size}
          style={{
            ...
            justifyContent: 'center',
            paddingInline: 4,
            ...(isTransparent && cfg.splitIconVariant === 'transparent' ? frostedStyles : {}),
          }}
        >
          {icon}
        </Badge>
  ...
  const fixedWidthBase = applyFixed
    ? cfg.iconPosition === 'hidden' || treatAsHiddenForWidth
      ? 40
      : 55
    : undefined;
