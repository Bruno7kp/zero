...
export const ChartItemStats: React.FC<ChartItemStatsProps> = ({
  stats,
  highlightWeek,
  chartId,
  chartType,
      ...
      <Grid justify="center" mb="md">
        <Grid.Col span={{ base: 12 }}>
          <Group justify="center" gap={10} mb="md">
            ...
            <Text fw={800} mb={2} size="xs" ta="center" tt="uppercase">
              {t('charts.stats.run')}
            </Text>
          </Group>
      ...
      <Grid justify="center" mb="md">
        ...
        {['artist', 'album', 'track'].includes(chartType) && (
          <LastfmPlaysBox
            ...
            entityName={
              chartType === 'artist' ? '' : entityName || stats.name || stats.entityName || ''
            }
            artistName={
              chartType === 'artist'
        ...
        {['album', 'track'].includes(chartType) &&
          (() => {
            const gold =
              chartType === 'track' ? chart?.music_gold_value || 0 : chart?.album_gold_value || 0;
            const platinum =
              chartType === 'track'
                ? chart?.music_platinum_value || 0
                : chart?.album_platinum_value || 0;
            const diamond =
              chartType === 'track'
                ? chart?.music_diamond_value || 0
                : chart?.album_diamond_value || 0;
            ...
            return (
              <Grid.Col span={{ base: 12, sm: 4 }}>
                <CertificationBadge
                  ...
                  totals={totals}
                  entity={{
                    name: entityName || stats.name || stats.entityName || '',
                    artistName: entityArtistName || stats.artistName || stats.artist || '',
                  }}
...
function LastfmPlaysBox({
  ...
  React.useEffect(() => {
    ...
    getUserPlaycountCached({
      ...
      .then((v: number) => {
        if (!canceled) setPlays(v);
      })
      .catch(() => {
        if (!canceled) setPlays(0);
      });
    return () => {
      canceled = true;
    };
