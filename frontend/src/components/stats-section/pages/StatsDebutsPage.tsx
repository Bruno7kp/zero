...
export const StatsDebutsPage: React.FC<{ position?: string; type?: string }> = ({ 
    position = 'all', 
    ...
    const [data, setData] = useState<DebutsRow[]>([]);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setData(result as DebutsRow[]);
            } catch (error) {
                console.error('Error loading debuts data:', error);
                setData([]);
            } finally {
                setLoading(false);
            }
    ...
    const columns: DataTableColumn<DebutsRow>[] = [
        {
            accessor: 'image',
            title: '',
            render: (row) => (
    ...
    return (
        <Stack gap="md" p="md">
            <div>
                <Title order={2}>
                    {t('stats.strongestDebuts', { defaultValue: 'Strongest Debuts' })}
                </Title>
                <Text c="dimmed">
                    {t('stats.strongestDebutsDesc', { defaultValue: 'Best first week performances' })}
                </Text>
            </div>
