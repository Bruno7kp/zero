export const generateGridHTML = (
  ...
  const containerStyle = `font-family: Arial, sans-serif; background: #fff; color: #000; width: ${sizePx}px; height: ${sizePx}px; box-sizing: border-box; display: grid; grid-template-columns: repeat(${size}, 1fr); grid-template-rows: repeat(${size}, 1fr); gap: 0px;`;
  ...
  data.forEach((row: any) => {
    ...
    let iconHtml = '';
    if (deltaRank === 'NEW') {
      iconHtml = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#13b4ff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="8" y1="12" x2="16" y2="12"></line></svg>';
