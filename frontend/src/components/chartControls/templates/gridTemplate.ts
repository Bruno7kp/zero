export const generateGridHTML = (
  ...
  data.forEach((row: any) => {
    ...
    const imageHtml = imageUrl
      ? `<img src="${imageUrl}" alt="" crossorigin="anonymous" onerror="this.style.display='none'" style="width: 100%; height: 100%; object-fit: cover; position: absolute; top: 0; left: 0;" />`
      : '<div style="width: 100%; height: 100%; background: #ccc; position: absolute; top: 0; left: 0;"></div>';
    const overlayHtml = showText
      ? `<div style="position: absolute; bottom: -1px; left: 0; right: 0; background: rgba(0,0,0,0.5); color: white; padding: 5px 10px; font-size: 14px; font-family: 'Inter', sans-serif; height: 55px; display: flex; align-items: center;">
      <div style="display: flex; align-items: center; gap: 5px; width: 100%;">
        ...
        <div style="display: flex; flex-direction: column; flex: 1; gap:0px${
          showVariationIcons ? '' : '; margin-left: 10px'
        }">
          <span style="font-size: 17px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(
            row.name || ''
          )}</span>
          <span style="font-size: 17px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(
            row.artistName || ''
