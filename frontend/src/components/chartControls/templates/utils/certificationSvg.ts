...
export function generateCertificationSvg(level: DiscLevel, multiplier?: number, size: number = 24): string {
  const stops = (
    level === 'gold'
      ? [
          { o: 0, c: '#7a5c00' },
  ...
  // Main disc
  svg += `<circle cx="${r}" cy="${r}" r="${r - 0.5}" fill="url(#${gradId})" mask="url(#${maskId})"/>`;
  svg += `<circle cx="${r}" cy="${r}" r="${r - 0.5}" fill="url(#${shineId})" mask="url(#${maskId})"/>`;
  ...
  for (let i = 0; i < grooveCount; i++) {
    ...
    const baseDark = level === 'none' ? 0.10 : 0.08;
    ...
    svg += `<circle cx="${r}" cy="${r}" r="${gr + 0.5}" fill="none" stroke="#ffffff" stroke-opacity="${lightOpacity}" stroke-width="0.35"/>`;
  }
  svg += '</g>';
  
  // Outer ring and hole ring
  svg += `<circle cx="${r}" cy="${r}" r="${r - 0.75}" fill="none" stroke="rgba(0,0,0,0.25)" stroke-width="0.75"/>`;
  svg += `<circle cx="${r}" cy="${r}" r="${holeR + 0.4}" fill="none" stroke="rgba(0,0,0,0.35)" stroke-width="0.6"/>`;
