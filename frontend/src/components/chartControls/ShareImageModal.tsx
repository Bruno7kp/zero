...
export const ShareImageModal: React.FC<ShareImageModalProps> = ({
  ...
  const [selectedType, setSelectedType] = useState<
    'grid' | 'stories' | 'stories2' | 'completo' | 'text'
  >(savedSettings.selectedType || 'stories2');
  const [selectedGridSize, setSelectedGridSize] = useState<3 | 4 | 5 | 6 | 7 | 8 | 9 | 10>(
    savedSettings.selectedGridSize || 3
  );
  const [selectedGridShowText, setSelectedGridShowText] = useState<boolean>(
    savedSettings.selectedGridShowText ?? true
  );
  const [selectedGridShowVariationIcons, setSelectedGridShowVariationIcons] = useState<boolean>(
    savedSettings.selectedGridShowVariationIcons ?? true
  );
  const [selectedStoriesTop, setSelectedStoriesTop] = useState<5 | 10>(
    savedSettings.selectedStoriesTop || 10
  );
  const [selectedStoriesPrimaryColor, setSelectedStoriesPrimaryColor] = useState<string>(
    savedSettings.selectedStoriesPrimaryColor || '#ff6d68'
  );
  const [selectedStoriesHighlightColor, setSelectedStoriesHighlightColor] = useState<string>(
    savedSettings.selectedStoriesHighlightColor || '#ffffff'
  );
  const [selectedStories2Top, setSelectedStories2Top] = useState<5 | 10>(
    savedSettings.selectedStories2Top || 10
  );
  const [selectedStories2BackgroundType, setSelectedStories2BackgroundType] = useState<
    'blur' | 'solid'
  >(savedSettings.selectedStories2BackgroundType || 'blur');
  const [selectedStories2BackgroundColor, setSelectedStories2BackgroundColor] = useState<string>(
    savedSettings.selectedStories2BackgroundColor || '#1a1a1a'
  );
  const [selectedStories2ShowPlays, setSelectedStories2ShowPlays] = useState<
    'last' | 'plays' | 'peak' | 'weeks'
  >(savedSettings.selectedStories2ShowPlays || 'last');
  const [selectedStories2ListWrapBackgroundType, setSelectedStories2ListWrapBackgroundType] =
    useState<'transparent' | 'solid'>(
      savedSettings.selectedStories2ListWrapBackgroundType || 'transparent'
    );
  const [selectedStories2ListWrapBackgroundColor, setSelectedStories2ListWrapBackgroundColor] =
    useState<string>(savedSettings.selectedStories2ListWrapBackgroundColor || '#1a1a1a');
  const [selectedStories2ShowAlbumCovers, setSelectedStories2ShowAlbumCovers] = useState<boolean>(
    savedSettings.selectedStories2ShowAlbumCovers ?? true
  );
  const [selectedStories2ShowColoredIcons, setSelectedStories2ShowColoredIcons] = useState<boolean>(
    savedSettings.selectedStories2ShowColoredIcons ?? true
  );
  const [selectedStories2ShowIconBackground, setSelectedStories2ShowIconBackground] =
    useState<boolean>(savedSettings.selectedStories2ShowIconBackground ?? true);
  const [selectedCompletoBackgroundColor, setSelectedCompletoBackgroundColor] = useState<string>(
    savedSettings.selectedCompletoBackgroundColor || '#1a1a1a'
  );
  const [selectedCompletoTop, setSelectedCompletoTop] = useState<string>(
    savedSettings.selectedCompletoTop || 'full'
  );
  const [selectedCompletoShowColoredIcons, setSelectedCompletoShowColoredIcons] = useState<boolean>(
    savedSettings.selectedCompletoShowColoredIcons ?? true
  );
  const [selectedCompletoColumns, setSelectedCompletoColumns] = useState<string[]>(
    savedSettings.selectedCompletoColumns || ['plays', 'last']
  );
  const [selectedCompletoCustomHeaderImage, setSelectedCompletoCustomHeaderImage] =
    useState<string>(savedSettings.selectedCompletoCustomHeaderImage || '');
  const [selectedCompletoShowCert, setSelectedCompletoShowCert] = useState<boolean>(
    savedSettings.selectedCompletoShowCert ?? false
  );
  ...
  const generatePreviewImage = useCallback(async () => {
      ...
      enrichedData = await Promise.all(
        chartData.map(async row => {
          ...
          if (existingStats?.peak?.position && existingStats?.totals?.withinCutoff) {
            ...
            try {
              const dbStats = await db.charts_stats.get([
                chart.id.toString(),
                chartType,
                row.entityId,
              ]);
              if (dbStats) {
                ...
                const historicalData = await db.charts_data
                  ...
                  .between(
                    [chart.id.toString(), chartType, row.entityId, '0000'],
                    [chart.id.toString(), chartType, row.entityId, week]
                  )
                  .toArray();
    ...
    if (selectedType === 'stories2') {
      ...
      htmlForCanvas = await generateStories2HTML(
        enrichedData,
        selectedStories2Top,
        week,
        weekNumber,
      ...
      const validCustomHeaderImage =
        selectedCompletoCustomHeaderImage && isAllowedImageDomain(selectedCompletoCustomHeaderImage)
          ? selectedCompletoCustomHeaderImage
          : '';
  ...
  const handleDownload = () => {
    ...
    const suffix =
      selectedType === 'grid'
        ? `_${selectedGridSize}x${selectedGridSize}`
        : selectedType === 'stories' || selectedType === 'stories2'
        ? `_${selectedType === 'stories' ? selectedStoriesTop : selectedStories2Top}`
        : '';
  ...
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      title={t('charts.share.shareChartTitle', 'Compartilhar chart')}
      fullScreen
      centered
    >
      <Box>
        <Grid>
          ...
          <Grid.Col span={{ base: 12, md: 8 }}>
            <Text size="sm" c="dimmed" mb="md">
              {t('charts.share.preview', 'Preview')}
            </Text>
            <Center
              style={{
            ...
            >
              {selectedType === 'text' ? (
                ...
                <>
                  ...
                  {!isLoading && (!previewImageUrl || currentImageType !== getCurrentTypeKey()) && (
                    <Text>Não foi possível gerar a prévia.</Text>
                  )}
                </>
              )}
            </Center>
