...
import { ChartWeekCard } from './cards/ChartWeekCard';
...
export const ChartsWeeksTimeline: React.FC<ChartsWeeksTimelineProps> = React.memo(
  ({ weeksData, themeMode, itemsPerPage = 50 }) => {
          ...
          <Box style={{ width: '100%', maxWidth: 800 }}>
            <Timeline active={paginatedData.length} bulletSize={32} lineWidth={2}>
              {paginatedData.map((weekData, index) => {
                ...
                const bullet =
                  hasAllKill && weekData.artistTop1 ? (
                    <AllKillBullet
                      entityId={weekData.artistTop1.entityId}
                      name={weekData.artistTop1.name}
                    />
                ...
                return (
                  <Timeline.Item key={weekData.week} bullet={bullet} lineVariant={lineVariant}>
                    <ChartWeekCard
                      week={weekData.week}
