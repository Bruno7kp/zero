...
export const ChartsWeeksGridView: React.FC<ChartsWeeksGridViewProps> = ({
  weeksData,
  itemsPerPage = 25,
  ...
  const cumulativeByWeek = useMemo(() => {
    const result: {
      artist: Record<string, Record<string, number>>;
    ...
    const sortedAsc = [...weeksData].sort(
      ...
      <Flex justify="center">
        <Box style={{ width: '100%', maxWidth: containerMaxWidth }}>
          <SimpleGrid
            cols={{
              base: 1,
              sm: typeFilter.length === 3 ? 1 : typeFilter.length === 2 ? 2 : 2,
              lg: typeFilter.length === 3 ? 1 : typeFilter.length === 2 ? 2 : 3,
            }}
            spacing="lg"
          >
              ...
              if (typeFilter.includes('artist') && weekData.artistTop1) {
                tiles.push(
                  <GridItem
                    ...
                    type="artist"
                    timesAtTop1={
                      cumulativeByWeek.artist[weekData.week]?.[weekData.artistTop1.entityId] || 1
                    }
                    onOpenModal={(row, url) => {
              ...
              if (typeFilter.includes('album') && weekData.albumTop1) {
                tiles.push(
                  <GridItem
                    ...
                    type="album"
                    timesAtTop1={
                      cumulativeByWeek.album[weekData.week]?.[weekData.albumTop1.entityId] || 1
                    }
                    onOpenModal={(row, url) => {
                      setImageModalRow(row);
                      setImageModalUrl(url || '');
              ...
              if (typeFilter.includes('track') && weekData.trackTop1) {
                tiles.push(
                  <GridItem
                    ...
                    type="track"
                    timesAtTop1={
                      cumulativeByWeek.track[weekData.week]?.[weekData.trackTop1.entityId] || 1
                    }
                    onOpenModal={(row, url) => {
                      setImageModalRow(row);
                      setImageModalUrl(url || '');
                ...
                <Flex key={weekData.week} direction="column" gap={8}>
                  <Flex align="center" justify="center" gap={8} wrap="wrap">
                    <Tooltip label={dateRange} withArrow>
                      <Text fw={600} size="lg">
                        {t('charts.weekNumber')}: {weekData.weekNumber}
                      </Text>
                    </Tooltip>
                    {typeFilter.length === 3 && hasAllKill && <AllKillBadge />}
                    <Button
                      size="xs"
                      variant="light"
                      px={6}
