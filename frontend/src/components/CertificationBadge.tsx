  ...
  if (!result) {
      ...
      return (
        <Card p="sm" withBorder>
          <Text size="xs" c="red" ta="center">
            {t('charts.stats.needOnlineForCert')}
          </Text>
        </Card>
      );
    }
    if (playsWeight > 0 && !username) {
      return (
        <Card p="sm" withBorder>
          <Text size="xs" c="red" ta="center">
            Last.fm username ausente no chart
          </Text>
        </Card>
      );
    }
    return (
      <Card p="sm" withBorder>
        <Text size="xs" ta="center">
          {loading ? t('charts.stats.loading') : t('charts.stats.noData')}
        </Text>
      </Card>
    );
  }
          ...
          {remainingToNext !== null && nextTarget !== null && (
            <Tooltip
              label={
                nextType === 'same' && nextMultiple && nextLevel
                  ? t('charts.stats.nextAt', {
                      value: `${nextMultiple}x ${t('values.' + nextLevel)}`,
                    })
                  : nextLevel
            ...
            >
              <div>
                ...
                <Text size="10px" mt={3}>
                  {nextType === 'same' && nextMultiple && nextLevel
                    ? t('charts.stats.remainingToSame', {
                        value: formatNumber(Math.max(0, Math.ceil(remainingToNext))),
                        multiple: nextMultiple,
                        level: t('values.' + nextLevel),
                      })
                    : nextLevel
                    ? t('charts.stats.remainingToHigher', {
                        value: formatNumber(Math.max(0, Math.ceil(remainingToNext))),
                        level: t('values.' + nextLevel),
                      })
                    : t('charts.stats.remaining', {
                        value: formatNumber(Math.max(0, Math.ceil(remainingToNext))),
                      })}
                </Text>
              </div>
