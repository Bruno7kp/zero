...
export const ImageEditModal: React.FC<ImageEditModalProps> = ({
  ...
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      title={t('imageEditModal.titleWithName', {
        name: name || t('imageEditModal.title', 'Imagem'),
      })}
      size="lg"
    ...
    >
        ...
        <Box mb="md" style={{ display: 'flex', justifyContent: 'center' }}>
          <img
            ...
            style={{
              ...
              maxHeight: 320,
              borderRadius: 8,
              border: isSquare ? undefined : '2px solid red',
            }}
          />
        </Box>
      )}
      {!isSquare && (
        <Text c="red" size="sm" mb="sm">
          {t('imageEditModal.squareWarning', 'A imagem deve ser quadrada!')}
        </Text>
      )}
      <Box mb="xs">
        <Text c="dimmed" size="xs" style={{ marginBottom: 4 }}>
          {t(
            'imageEditModal.domainNotice',
            'Por segurança, só é possível usar imagens de sites confiáveis como Last.fm, Spotify, Imgur, Unsplash, etc. Caso queira usar uma imagem própria, faça upload em um serviço como Imgur ou Cloudinary e cole o link aqui.'
          )}
        </Text>
      </Box>
      <Box mb="sm">
      ...
      <Group>
        <button
          style={{
            ...
            border: 'none',
            fontWeight: 600,
            cursor: loading || !imgPreview || !isSquare ? 'not-allowed' : 'pointer',
          }}
          disabled={loading || !imgPreview || !isSquare || !!domainError}
          onClick={handleSave}
        >
          {t('imageEditModal.save', 'Salvar')}
        </button>
        <button
          style={{
            padding: '6px 16px',
        ...
        >
          {t('imageEditModal.fetchSpotify', 'Buscar do Spotify')}
        </button>
      </Group>
      {domainError ? (
        <div style={{ color: theme.colors.red[6], fontSize: 13, marginTop: 8 }}>{domainError}</div>
      ) : null}
    </Modal>
  );
};
