...
export const TrackAchievements: React.FC<TrackAchievementsProps> = ({
  stats,
  chartRun,
  background,
}) => {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    return storage.getJson<boolean>(KEYS.TRACK_ACHIEVEMENTS_COLLAPSED, [], false) ?? false;
  });

  useEffect(() => {
    storage.setJson(KEYS.TRACK_ACHIEVEMENTS_COLLAPSED, collapsed);
  }, [collapsed]);
  ...
  const achievements = useMemo<Achievement[]>(() => {
    ...
    result.push({
      ...
      title: t('library.achievements.track.risingStar'),
      description: hasRisingStar
        ? t('library.achievements.track.risingStarDesc', { rank: peakPosition, target: 10 })
        : t('library.achievements.track.risingStarLocked', { target: 10 }),
    ...
    result.push({
      ...
      title: t('library.achievements.track.firstNumberOne'),
      description: hasNumberOne
        ? t('library.achievements.track.firstNumberOneDesc', { weeks: weeksToFirst })
        : t('library.achievements.track.firstNumberOneLocked'),
    ...
    result.push({
      ...
      title: t('library.achievements.track.chartDominator'),
      description: hasDominator
        ? t('library.achievements.track.chartDominatorDesc', { weeks: weeksAtOne, target: 5 })
        : t('library.achievements.track.chartDominatorProgress', {
            current: weeksAtOne,
            target: 5,
          }),
    ...
    result.push({
      ...
      title: t('library.achievements.track.consistencyKing'),
      description: hasConsistency
        ? t('library.achievements.track.consistencyKingDesc', { weeks: maxConsecutive, target: 10 })
        : t('library.achievements.track.consistencyKingProgress', {
    ...
    // 5. Veteran - 26 weeks in charts (Epic)
    const hasVeteran = totalWeeks >= 26;
    result.push({
      id: 'veteran',
      icon: <IconMedal size={24} />,
      title: t('library.achievements.track.veteran'),
      description: hasVeteran
        ? t('library.achievements.track.veteranDesc', { weeks: totalWeeks, target: 26 })
        : t('library.achievements.track.veteranProgress', {
            current: totalWeeks,
            target: 26,
          }),
      unlocked: hasVeteran,
      progress: Math.min(totalWeeks, 26),
      total: 26,
      color: 'teal',
      rarity: 'epic',
    });
    ...
    result.push({
      ...
      title: t('library.achievements.track.pointMaster'),
      description: hasPointMaster
        ? t('library.achievements.track.pointMasterDesc', {
            points: totalPoints.toLocaleString(),
            target: (1000).toLocaleString(),
          })
        : t('library.achievements.track.pointMasterProgress', {
