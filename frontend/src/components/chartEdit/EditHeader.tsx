...
export const EditHeader: React.FC<EditHeaderProps> = ({ type, week, cutoff, loading, canRefresh, t, onRefresh }) => {
  return (
    <Group justify="space-between" mb={6} gap="xs">
      ...
      <Group gap="xs">
        <Tooltip label={t('chartEdit.refreshTip', 'Atualizar desta semana no Last.fm (inclui empates fora do cutoff)')}>
          <Button size="xs" variant="default" leftSection={<IconRefresh size={14} />} onClick={onRefresh} disabled={loading || !canRefresh}>{t('chartEdit.refresh', 'Atualizar da fonte')}</Button>
        </Tooltip>
      </Group>
    </Group>
  );
