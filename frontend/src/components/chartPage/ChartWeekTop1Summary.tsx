...
export const ChartWeekTop1Summary: React.FC<ChartWeekTop1SummaryProps> = ({
  chartId,
  week,
  refreshKey,
}) => {
  ...
  useEffect(() => {
    ...
    async function fetchTop1() {
      try {
        let targetWeek = week;
        if (!targetWeek) {
          const all = await db.charts_data.where('chartId').equals(chartId).toArray();
          const weeks = Array.from(new Set(all.map(i => i.week))).sort(
        ...
        const results = await Promise.all(
          types.map(async type => {
            ...
            return top
              ? { type, name: top.name, artistName: top.artistName, entityId: top.entityId }
              : null;
          })
        );
