...
export const RankCellList: React.FC<{
  ...
  <Flex direction="column" align="center" style={{ minWidth: 48, maxWidth: 48, flex: '0 0 48px' }}>
    <Text fw={600} size={scaleSize('xl')} className={row.rank === 1 ? 'peak' : undefined}>
      {row.rank}
    </Text>
    {showDeltaBadge && (
      <DeltaBadge
...
export const PlaysCellList: React.FC<{
  ...
  return (
    <Flex
      ...
      align="center"
      mr="sm"
      style={{ minWidth: 72, maxWidth: 72, flex: '0 0 72px' }}
    >
...
export const CombinedPeakWeeksBlock: React.FC<{
  ...
  >
    <Text
      size={scaleSize('xs')}
      tt="uppercase"
      style={{ lineHeight: 1, letterSpacing: 0.5, textAlign: 'right' }}
    >
      {t('charts.peak')}
    </Text>
    <Flex align="center" gap={6} style={{ justifySelf: 'start' }}>
      <Text
        fw={600}
        size={scaleSize('xl')}
        className={displayPeak === 1 ? 'peak' : undefined}
        style={{ transition: 'color 120ms ease', textAlign: 'left', lineHeight: 1.2 }}
      >
        {displayPeak != null ? (
          displayPeak
        ) : (
...
export const PeakCellList: React.FC<{
  ...
  <Flex
    ...
    align="center"
    mr="sm"
    style={{ minWidth: 48, maxWidth: 48, flex: '0 0 48px' }}
  >
    <Text
      fw={600}
      size={scaleSize('xl')}
    ...
    {showPeakCount && display === 1 && renderedCountAtOne != null ? (
      <Text
        c="dimmed"
        mt={2}
        style={{ lineHeight: 1, letterSpacing: 0.5, fontSize: '0.6em' }}
      >{`${renderedCountAtOne}x`}</Text>
    ) : rank === 1 ? (
      <Text
        c="dimmed"
