...
export const ChartWeekListRow: React.FC<{
  ...
  ({
    ...
    const playsVariationLocation = (useSelector(
      (state: any) => state.columns?.views?.list?.settings?.playsVariationLocation
    ) || 'under') as 'hidden' | 'under' | 'column';
    const playsVariationDisplay = (useSelector(
      (state: any) => state.columns?.views?.list?.settings?.playsVariationDisplay
    ...
    const peakCountStyle =
      useSelector((state: any) => state.columns?.views?.list?.settings?.peakCountStyle) ||
      'noCount';
    const showPeakCount = peakCountStyle === 'withCount';
    const showFormulaInsteadOfPlays = (useSelector(
      (state: any) => state.columns?.views?.list?.settings?.showFormulaInsteadOfPlays
    ) || false) as boolean;
    ...
    useEffect(() => {
      try {
        ...
        for (const [entityId, s] of Object.entries(globalStatsMap || {})) {
          ...
          if (weeksAtPeak != null && lastWeeksAtPeakById[entityId] !== weeksAtPeak) {
            lastWeeksAtPeakById[entityId] = weeksAtPeak;
            changed = true;
          }
        }
        if (changed) {
          ...
          setLastWeeksAtPeakById({ ...lastWeeksAtPeakById });
        }
      } catch {
        /* noop */
      }
    ...
    const effectiveFontScale = isDropped
      ? (Math.max(-2, fontScale - 1) as -2 | -1 | 0 | 1 | 2)
      : fontScale;
            ...
            {filteredColumns.map((col: any) => {
              ...
              if (col.key === 'peak') {
                ...
                if (listPeakWeeksCombined && alsoHasWeeks) {
                  ...
                  const displayPeak =
                    currentPeak != null ? currentPeak : stablePeak != null ? stablePeak : undefined;
                  ...
                  const renderedCountAtOne =
                    displayPeak === 1
                      ? hasStats
                        ? Math.max(1, (rawCountAtOne as number) ?? 1)
                        : 1
                      : null;
                  return (
                    <CombinedPeakWeeksBlock
              ...
              if (col.key === 'altVariation' && showAltVariationRedux) {
                return (
                  <AltVariationCellList
                    key={col.key}
                    value={(altVariation ? altVariation(row, idx) : undefined) as any}
                    cfg={badgeStylesRank}
                  />
                );
              }
