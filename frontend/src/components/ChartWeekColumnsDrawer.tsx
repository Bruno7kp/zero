...
import {
  updateColumn,
  COLUMNS_DEFAULT_COLUMNS as defaultColumns,
  resetColumns,
  setContainerSize,
  setRankVariationLocation,
  setPlaysVariationDisplay,
  setTableBackground,
  setListBackground,
  setArtistDisplayMode,
  setPlaysVariationLocation,
  setPeakCountStyle,
  setFontScale,
  setListPeakWeeksCombined,
  setShowCarousel,
  setShowDroppedItems,
  setShowFormulaInsteadOfPlays,
...
export const ChartWeekColumnsDrawer: React.FC<ChartWeekColumnsDrawerProps> = ({
  viewType,
  onColumnsChange,
  opened,
  onOpenedChange,
  ...
  const showFormulaInsteadOfPlays =
    (viewConfig?.settings as any)?.showFormulaInsteadOfPlays || false;
  ...
  const open = () => {
    if (onOpenedChange) onOpenedChange(true);
    else setInternalOpened(true);
  };
  const close = () => {
  ...
  const columnsWithVisibility = defaultColumns.map(
    (col: { key: string; label: string; labelComplete?: string; visible: boolean }) => {
  ...
  useEffect(() => {
    const rankPreset = badgeStyles?.views?.[viewType]?.rank?.preset;
    if (
      (rankPreset === 'maximalist' || rankPreset === 'maximalistLight') &&
      !specialsAllowedForRank
    ) {
      ...
      >
        ...
        >
                ...
                <Accordion.Control>
                  <Flex direction="column" gap={2}>
                    <Flex align="center" gap={8}>
                      ...
                      <Text fw={600}>{t('charts.general')}</Text>
                    </Flex>
                    <Text size="xs" c="dimmed">
                      {t('charts.drawer.generalDescription')}
                    </Text>
                  </Flex>
                </Accordion.Control>
                <Accordion.Panel>
                  <LayoutSection
                    ...
                    onFontScaleChange={v =>
                      dispatch(setFontScale({ view: viewType, scale: v as any }))
                    }
                    ...
                    onShowDroppedItemsChange={v =>
                      dispatch(setShowDroppedItems({ view: viewType, show: v }))
                    }
                    onShowCarouselChange={v =>
                      dispatch(setShowCarousel({ view: viewType, show: v }))
                    }
                  />
                </Accordion.Panel>
              </Accordion.Item>
              ...
              <Accordion.Item value="columns">
                <Accordion.Control>
                  <Flex direction="column" gap={2}>
                    <Flex align="center" gap={8}>
                      <IconColumns size={16} />
                      <Text fw={600}>{t('charts.columns')}</Text>
                    </Flex>
                    <Text size="xs" c="dimmed">
                      {t('charts.drawer.columnsDescription')}
                    </Text>
                  </Flex>
                </Accordion.Control>
                <Accordion.Panel>
                  <ColumnsSection
                    ...
                    onToggleColumn={(key, visible) =>
                      dispatch(updateColumn({ view: viewType, key, visible }))
                    }
                    onToggleListPeakWeeksCombined={combined =>
                      dispatch(setListPeakWeeksCombined({ combined }))
                    }
                    onArtistDisplayModeChange={mode =>
                      dispatch(setArtistDisplayMode({ view: 'table', mode }))
                    }
                    peakMode={
                      (columnsWithVisibility.find(c => c.key === 'peak')?.visible
                        ? (viewConfig?.settings?.peakCountStyle || 'noCount') === 'withCount'
                          ? 'showWithCount'
                          : 'show'
                        : 'hide') as any
                    }
                    onPeakModeChange={v => {
                      if (v === 'hide') {
                        ...
                        dispatch(
                          setPeakCountStyle({
                            view: viewType,
                            mode: v === 'showWithCount' ? 'withCount' : 'noCount',
                          })
                        );
                      }
                    }}
                    showFormulaInsteadOfPlays={showFormulaInsteadOfPlays}
                    onToggleShowFormulaInsteadOfPlays={show =>
                      dispatch(setShowFormulaInsteadOfPlays({ view: viewType, show }))
                    }
                  />
                </Accordion.Panel>
              </Accordion.Item>
                ...
                <Accordion.Control>
                  <Flex direction="column" gap={2}>
                    <Flex align="center" gap={8}>
                      ...
                      <Text fw={600}>{t('charts.variations')}</Text>
                    </Flex>
                    <Text size="xs" c="dimmed">
                      {t('charts.drawer.variationDescription')}
                    </Text>
                  </Flex>
                </Accordion.Control>
                <Accordion.Panel>
                  <VariationsSection
                    ...
                    playsVariationDisplay={
                      ((viewConfig?.settings as any)?.playsVariationDisplay || 'percent') as any
                    }
                    onRankLocationChange={loc =>
                      dispatch(setRankVariationLocation({ view: viewType, location: loc as any }))
                    }
                    onPlaysLocationChange={loc =>
                      dispatch(
                        setPlaysVariationLocation({ view: viewType as any, location: loc as any })
                      )
                    }
                    onPlaysDisplayChange={display =>
                      dispatch(setPlaysVariationDisplay({ view: viewType as any, display }))
                    }
                  />
                </Accordion.Panel>
              </Accordion.Item>
                ...
                <Accordion.Control>
                  <Flex direction="column" gap={2}>
                    <Flex align="center" gap={8}>
                      ...
                      <Text fw={600}>{t('charts.badgeStyles.title')}</Text>
                    </Flex>
                    <Text size="xs" c="dimmed">
                      {t('charts.drawer.badgesDescription')}
                    </Text>
                  </Flex>
                </Accordion.Control>
                <Accordion.Panel>
          ...
          <Group justify="space-between" px={4} pb={4} mt={4} style={{ flexShrink: 0 }}>
            <Button variant="light" size="xs" onClick={handleReset}>
              {t('common.resetToDefault')}
            </Button>
            <Button size="xs" onClick={close}>
              {isMobile ? t('common.closeAndSave') : t('common.close')}
            </Button>
          </Group>
        </Paper>
      </Drawer>
