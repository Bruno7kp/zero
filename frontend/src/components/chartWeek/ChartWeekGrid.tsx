...
export const ChartWeekGrid: React.FC<ChartWeekGridProps> = ({
  chart,
  week,
  type,
  clientId,
      ...
      if (stored && Array.isArray(stored)) {
        stored.forEach((col: any) => {
          dispatch({
            ...
            payload: { view: 'grid', key: col.key, visible: col.visible },
          });
        });
      }
    } catch {
  ...
  const rankVariationLocation = useSelector(
    (state: any) => state.columns?.views?.grid?.settings?.rankVariationLocation || 'under'
  );
  const peakCountStyle =
    useSelector((state: any) => state.columns?.views?.grid?.settings?.peakCountStyle) || 'noCount';
  const showPeakCount = peakCountStyle === 'withCount';
  const showDroppedItems =
    useSelector((state: any) => state.columns?.views?.grid?.settings?.showDroppedItems) || false;
  const showFormulaInsteadOfPlays =
    useSelector((state: any) => state.columns?.views?.grid?.settings?.showFormulaInsteadOfPlays) ||
    false;
  ...
  const safeDisplayedData =
    displayedData && displayedData.length > 0 ? displayedData : prevDataRef.current;
  const statsMap = useSelector((state: any) => state.charts.statsMap);
  const columns = useSelector(
    (state: any) => state.columns?.views?.grid?.columns || state.columns?.columns || []
  );
  ...
  useEffect(() => {
    if (!data.length || !week || !chart?.id) return;
    const wantsStats = columns.some(
      (c: any) => (c.key === 'peak' || c.key === 'totalWeeks') && c.visible
    );
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        const id = setTimeout(() => {
          if (cancelled) return;
          dispatch(
            fetchStatsMapIncremental({ chartId: `${chart.id}`, chartType: type, data, week })
          );
        }, 900);
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true;
      if ((window as any).__gridStatsDroppedTimer)
        clearTimeout((window as any).__gridStatsDroppedTimer);
    };
  ...
  const modalTitle = modalRow
    ? `${modalRow.name}${modalRow.artistName ? ' — ' + modalRow.artistName : ''}`
    : 'Detalhes';
  return (
    <>
      <Modal
        ...
        {visibleCards.map((row: ChartData, idx: number) => {
          ...
          return (
            <GridItemRenderer
              ...
              onImageChange={() => {
                if (row.entityId) {
                  setImageForceUpdate(fu => ({
                    ...fu,
                    [row.entityId]: (fu[row.entityId] || 0) + 1,
                  }));
                }
              }}
              ...
              cornerOverlay={
                rankVariationLocation === 'corner' ? (
                  <GridAltVariationCorner
                    row={row}
                    idx={idx}
              ...
              stats={{
                peak: {
                  position:
                    (stats?.peak?.position != null
                      ? stats?.peak?.position
                      : lastPeakById[row.entityId]) ?? undefined,
                  weeksAtPeak:
                    (stats?.peak?.weeksAtPeak != null
                      ? stats?.peak?.weeksAtPeak
                      : lastWeeksAtPeakById[row.entityId]) ?? undefined,
                },
                totals: {
                  withinCutoff:
                    (stats?.totals?.withinCutoff != null
                      ? stats?.totals?.withinCutoff
      ...
      {showLoadingTail && (
        <Box py="sm" style={{ textAlign: 'center' }}>
          <Text size="xs" c="dimmed">
            Carregando {visibleCards.length}/{progressive.total}…
          </Text>
        </Box>
      )}
            ...
            {droppedItems.map((row: ChartData, idx: number) => {
              ...
              const formulaValue =
                ...
                (type === 'album' || type === 'track' || type === 'artist')
                  ? calculateWeekFormulaValue({
                      chart,
                      chartType: (type === 'artist' ? 'artist' : type) as
                        | 'album'
                        | 'track'
                        | 'artist',
                      rank: row.rank,
              ...
              return (
                <GridItemRenderer
                  ...
                  imageForceUpdate={imageForceUpdate[row.entityId]}
                  lastImageUrl={lastImageUrlByEntityId[row.entityId]}
                  colSpan={{ base: 15, sm: 10, md: 7.5, lg: 5 }}
                  onImageChange={() => {
                    if (row.entityId) {
                      setImageForceUpdate(fu => ({
                        ...fu,
      ...
      <ImageEditModal
        ...
        onImageChange={(url: string) => {
          ...
          if (imageModalRow?.entityId) {
            setImageForceUpdate(fu => ({
              ...fu,
              [imageModalRow.entityId]: (fu[imageModalRow.entityId] || 0) + 1,
            }));
          }
        }}
      />
