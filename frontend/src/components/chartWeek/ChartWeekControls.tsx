...
export const ChartWeekControls: React.FC<ChartWeekControlsProps> = ({
  chart,
  week,
  type,
  onChange,
  ...
  const [view, setView] = React.useState<'table' | 'grid' | 'list'>(() => {
    try {
      ...
      return saved === 'table' || saved === 'grid' || saved === 'list'
        ? (saved as 'table' | 'grid' | 'list')
        : 'table';
    } catch {
      return 'table';
    }
  ...
  const handleSetView = (v: 'table' | 'grid' | 'list') => {
    setView(v);
    try {
      storage.set(KEYS.CHART_WEEK_VIEW, v);
    } catch {
      /* ignore */
    }
    ...
    <>
      <Grid mt="md">
        ...
        <Grid.Col span={{ base: 5, sm: 4 }} style={{ display: 'flex', alignItems: 'center' }}>
          <Button onClick={handlePrev} size="xs" variant="subtle" px={6} disabled={!prev || isBusy}>
          ...
          <WeekPicker
            ...
            disabled={isBusy}
          />
          <Button onClick={handleNext} size="xs" variant="subtle" px={6} disabled={!next || isBusy}>
            <IconArrowRight size={18} />
          </Button>
        </Grid.Col>
        {/* Centro: seleção de tipo */}
        <Grid.Col span={{ base: 7, sm: 4 }}>
          <Flex align="center" justify={{ base: 'flex-end', sm: 'center' }} w="100%" gap="xs">
            {/* Mobile: show settings menu to the RIGHT of the type segmented control */}
            <TypeControl
              type={type}
              isBusy={isBusy}
              onChangeType={v => triggerChange(week || '', v)}
            />
            {/* Drawer control hidden trigger, controlled open */}
            <ChartWeekColumnsDrawer
              viewType={view}
              opened={drawerOpened}
