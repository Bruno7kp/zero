...
export const ChartWeekList: React.FC<ChartWeekListProps> = ({
  chart,
  week,
  type,
  altVariation,
  ...
  const columns = useSelector(
    (state: any) => state.columns?.views?.list?.columns || state.columns?.columns || []
  );
  const theme = useMantineTheme();
  const listBackground = useSelector(
    (state: any) => state.columns?.views?.list?.settings?.listBackground || 'default'
  );
  ...
  useEffect(() => {
    ...
    const wantsStats = columns.some(
      (c: any) =>
        (c.key === 'peak' || c.key === 'totalWeeks' || (c.key === 'cert' && type !== 'artist')) &&
        c.visible
    );
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true;
      if ((window as any).__listStatsDroppedTimer)
        clearTimeout((window as any).__listStatsDroppedTimer);
    };
      ...
      {showLoadingTail && (
        <Flex justify="center" py="sm">
          <Text size="xs" c="dimmed">
            Carregando {visibleRows.length}/{progressive.total}…
          </Text>
        </Flex>
      )}
