...
interface ChartsListCardProps {
  ...
  processingState?: Record<
    string,
    { clearing?: boolean; rebuilding?: boolean; progress?: number; total?: number }
  >; // opcional
}

const ChartsListCard = ({
  charts,
  t,
  openDeleteModal,
  isOnline,
          ...
          <Table.Thead>
            <Table.Tr>
              ...
              <Table.Th style={{ width: '40%' }}>
                {t('forms.createChart.lastfmUsernameLabel')}
              </Table.Th>
          ...
          <Table.Tbody>
            {hasCharts ? (
              charts.map((chart: any) => {
                ...
                const progressPct =
                  rebuilding && proc?.total
                    ? Math.min(100, Math.round(((proc.progress || 0) / (proc.total || 1)) * 100))
                    : null;
                return (
                      ...
                      <Group gap={4} justify="flex-end" wrap="nowrap">
                        ...
                        <Menu withinPortal shadow="md" position="bottom-end">
                          ...
                          <Menu.Dropdown>
                            ...
                            >
                              {rebuilding && progressPct != null
                                ? `${t('settings.rebuildStats')} (${progressPct}%)`
                                : t('settings.rebuildStats')}
                            </Menu.Item>
                            <Menu.Divider />
                            <Menu.Item
                              ...
                              disabled={!isOnline || disabled}
                              onClick={() =>
                                isOnline && !disabled && openDeleteModal(chart.id, chart.name)
                              }
                            >
