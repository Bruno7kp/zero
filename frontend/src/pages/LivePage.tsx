...
import {
  ...
  getWeeklyTrackChart,
  getWeeklyAlbumChart,
  type FormattedChartItem,
} from '../services/lastfm.ts';
...
const LivePage = () => {
  ...
  const tableBgSetting = (useSelector(
    (state: any) => state.columns?.views?.table?.settings?.tableBackground
  ) || 'default') as 'default' | 'transparent';
  const paperProps =
    tableBgSetting === 'transparent'
      ? { shadow: 'none' as const, bg: 'transparent' as const }
      : { shadow: 'xs' as const };
  const artistMode: 'under' | 'column' = (useSelector(
    (state: any) => state.columns?.views?.table?.settings?.artistDisplayMode
  ) || 'under') as any;
  const showInlineImage = !!useSelector(
    (state: any) =>
      state.columns?.views?.table?.columns?.find((c: any) => c.key === 'image')?.visible
  );
  ...
  useEffect(() => {
    const fetchLiveChart = async () => {
      ...
      try {
        ...
        if (
          !activeChart ||
          activeChart.day_of_week === undefined ||
          !activeChart.timezone ||
          !activeChart.lastfm_username
        ) {
          throw new Error(
            "Dados do chart incompletos. Verifique 'day_of_week', 'timezone' e 'lastfm_username'."
          );
        }
        ...
        const cutoffVal =
          (activeChart as any)[cutoffKey] !== undefined ? (activeChart as any)[cutoffKey] : 100;
        ...
        try {
          ...
          if (prevWeek) {
            ...
            const keyOf = (name?: string, artist?: string) =>
              `${norm(name || '')}|${norm(artist || '')}`;
  ...
  useEffect(() => {
    const enrichNewRe = async () => {
      ...
      try {
        ...
        /* ignore fetch of previous weeks */
      }
      ...
      for (let i = 0; i < chartData.length; i++) {
        ...
        if (prevAnySet.has(rowKey)) updates.push({ idx: i, value: 'RE' });
        else updates.push({ idx: i, value: 'NEW' });
      }
      if (updates.length) {
        setChartData(prev =>
          prev.map((r, idx) => {
