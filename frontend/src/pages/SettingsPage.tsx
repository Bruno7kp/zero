...
function SettingsPage() {
      ...
      try {
        ...
        for (const cid of Object.keys(byChart)) {
          for (const wk of Object.keys(byChart[cid]).sort()) {
            ...
            await (db as any).chart_weeks.put({
              chartId: cid,
              week: wk,
              status: hasAll ? 'complete' : 'partial',
            });
  ...
  useEffect(() => {
    if (!isOnline) return;
    const last = syncState.lastFullChartsSync
      ? new Date(syncState.lastFullChartsSync).getTime()
      : 0;
    const now = Date.now();
  ...
  const openDeleteModal = (chartId: number, chartName: string) => {
    ...
    modals.openConfirmModal({
      ...
      labels: {
        confirm: t('forms.deleteChart.deleteButton'),
    ...
    <Container className="noPaddingMobile">
      <Flex direction="column" p="xs" gap="sm">
        ...
        <Group justify="space-between" align="center">
          <Group gap="xs">
            <Badge
              ...
              variant="light"
              leftSection={isOnline ? <IconCloudCheck size={14} /> : <IconCloudOff size={14} />}
            >
            ...
            {syncState.lastFullChartsSync && (
              <Tooltip label={syncState.lastFullChartsSync}>
                <Badge variant="light">
                  {t('settings.lastSync')}:{' '}
                  {new Date(syncState.lastFullChartsSync).toLocaleString()}
                </Badge>
              </Tooltip>
            )}
            {reindexing && (
              <Badge variant="dot" color="indigo">
                {t('settings.reindexingWeeks')}
              </Badge>
            )}
            {!reindexing && reindexedWeeks > 0 && (
              <Badge variant="light" color="indigo">
                {t('settings.reindexedWeeks', { count: reindexedWeeks })}
              </Badge>
            )}
          </Group>
          <Group gap="xs">
        ...
        <ActiveChartCard
          ...
          t={t}
          chartOptions={charts.map((chart: any) => ({
