...
import { getBestDebuts, getAllWeeks, getYearRange } from '../../../utils/statsQueries';
...
interface WeeklyDebutDatum extends BarDatum {
  entity: string;
  artistName: string;
  plays: number;
  week: string;
...
const TopWeeklyDebutsChart: React.FC = () => {
  ...
  const handleTypeChange = React.useCallback(
    (newType: 'track' | 'album' | 'artist') => {
      navigate(`/stats/visualizations/top-weekly-debuts/${newType}`);
    },
    [navigate]
  );
  const [loading, setLoading] = React.useState<boolean>(false);
  const [data, setData] = React.useState<WeeklyDebutDatum[]>([]);
  ...
  React.useEffect(() => {
    ...
    const loadMetadata = async () => {
      try {
        ...
        console.warn('[visualizations] failed to load metadata for weekly debuts', error);
        if (mounted) {
          setAllWeeks([]);
          setYearRange(null);
        }
  ...
  React.useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const normalized: WeeklyDebutDatum[] = debuts
        ...
        if (primarySlice.length) {
          fetchSpotifyImagesBatch(
            ...
            .catch(imageError => {
              console.warn('[visualizations] failed to load weekly debuts images', imageError);
            });
        }
      } catch (error) {
        console.error('[visualizations] failed to load weekly debuts', error);
        if (mounted) {
          setData([]);
          setHasLoadedOnce(true);
        }
        ...
        {loading && !hasLoadedOnce ? (
          ...
          <div style={{ height: Math.max(320, limitedData.length * 32) }}>
            <ResponsiveBar
              ...
              colors={({ data: datum }) =>
                (datum as WeeklyDebutDatum).barColor || theme.colors.blue[6]
              }
              enableGridX
              enableGridY={false}
              axisBottom={{
                tickSize: 0,
                tickPadding: 12,
                legend: t('stats.visualizations.weeklyDebuts.axisBottom'),
                legendOffset: 40,
              ...
              tooltip={({ indexValue, value }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      {t('stats.visualizations.weeklyDebuts.tooltipPlays', {
                        count: Number(value),
                      })}
                    </div>
                    <div>
                      {t('stats.visualizations.weeklyDebuts.tooltipWeek', {
                        week: datum?.week ?? '-',
                      })}
                    </div>
                    <div>
                      {t('stats.visualizations.weeklyDebuts.tooltipRank', {
                        rank: datum?.rank ?? '-',
                      })}
                    </div>
                  </div>
...
export default TopWeeklyDebutsChart;
