...
import { IconArrowLeft, IconMusic, IconChevronDown, IconChevronUp } from '@tabler/icons-react';
...
export const TrackDetailPage: React.FC = () => {
  ...
  const [chartRunCollapsed, setChartRunCollapsed] = useState<boolean>(() => {
    return storage.getJson<boolean>(KEYS.TRACK_CHART_RUN_COLLAPSED, [], false) ?? false;
  });

  useEffect(() => {
    storage.setJson(KEYS.TRACK_CHART_RUN_COLLAPSED, chartRunCollapsed);
  }, [chartRunCollapsed]);
