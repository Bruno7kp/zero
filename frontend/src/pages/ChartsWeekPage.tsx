...
export const ChartsWeekPage: React.FC = () => {
  ...
  const currentContainerSize =
    columnsState?.views?.[view]?.settings?.containerSize || (view === 'grid' ? 'xl' : 'md');
  ...
  const isSync =
    chart && selectedWeek === weekParam && selectedType === (typeParam || DEFAULT_TYPE);
  ...
  return (
    <>
      <Container
        ...
        size={isMobile ? '100%' : currentContainerSize}
        px="xs"
      >
        {chart && showCarousel && (
          <Suspense
        ...
        {!noChart && (
          <>
            ...
            {!loadingData && !hasAnyData && !isSwitchingTarget && (
              <Center py="md">
                <div style={{ opacity: 0.7, fontSize: 14 }}>Sem dados para esta semana.</div>
              </Center>
            )}
          </>
        )}
