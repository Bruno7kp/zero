...
function qa_get_logged_in_user()
{
	...
	session_start();
	$userid = null;
	if (isset($_SESSION["ZERO"]["B7KP\ENTITY\USER"])) 
	{
		$userid =  $_SESSION["ZERO"]["B7KP\ENTITY\USER"];	
	}

	else if(isset($_COOKIE["ZEROB7KP\ENTITY\USER"]))
	{
		$userid = $_COOKIE["ZEROB7KP\ENTITY\USER"];	
	}
	...
	$mod = array("theservano", "lucholerma", "pedromedeiros2", "katyanitta", "diogobalk7");
	$level = QA_USER_LEVEL_BASIC;
	if(in_array(mb_strtolower($result['login']), $mod))
	{
		$level = QA_USER_LEVEL_MODERATOR;
	}
	if(in_array(mb_strtolower($result['login']), $super))
	{
	...
	*/
		...
		if (count($publicusernames)) {
			...
			$results = qa_db_read_all_assoc(qa_db_query_raw(
				'SELECT login, id FROM user WHERE login IN (' . implode(',', $escapedusernames) . ')'
			));
...
function qa_get_public_from_userids($userids)
{
	...
	*/
		...
		if (count($userids)) {
			...
			$results = qa_db_read_all_assoc(qa_db_query_raw(
				'SELECT login, id FROM user WHERE id IN (' . implode(',', $escapeduserids) . ')'
			));
...
function qa_avatar_html_from_userid($userid, $size, $padding)
{
	...
	return '<img src="http://i.imgur.com/HIAFTNm.png" width=58>';
